...
let agent=async function(session){
	...
	let self;
	let InitBash,Get,Clone,GetPath,FindGuide,CheckGuide,NoReadme,WriteSetupGuide,SetupProject,FixArgs,Tip1,LLMCheckClone,CheckClone,Failure,Retry,goto,Navigate;
	/*#{1HDBOSUN90LocalVals*/
	let repo, folder;
...
请根据find命令的输出结果，识别并返回所有可能帮助项目安装、部署、构建和配置的文件路径。`},
	...
	segs["NoReadme"]=NoReadme=async function(input){//:1IVIRNSHQ0
		...
		let role="assistant";
		let content=(($ln==="CN")?("未找到部署指南，部署终止"):("Depolyment guide is not found, aborted."));
...
//						"text": {
//							"type": "string",
//							"valText": "Depolyment guide is not found, aborted.",
//							"localize": {
//								"EN": "Depolyment guide is not found, aborted.",
//								"CN": "未找到部署指南，部署终止"
//							},
//							"localizable": true
//						},
