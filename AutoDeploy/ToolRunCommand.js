...
let ToolRunCommand=async function(session){
	...
	let self;
	let FixArgs,Run,CheckSuccess,Check,Fix,Action,RunCommand,CheckFinish,Fail,GetPath,BackPath,Success,Issues,Search,RunSearch,SearchIssue,CheckNetwork,Network,Retry,ListFiles,Path,ReadFile,ModifyFile,FixSuccess;
	/*#{1IUIO756A0LocalVals*/
	let current_output, output, current_path, issue, search_result, bug, current_files, latest_path, all_files;
	/*}#1IUIO756A0LocalVals*/
...
4. 如果需要修改配置文件或其他文件来解决问题，可以修改文件，输出格式为 {"reason": "<推理过程>", "action": "modify", "content": "<文件绝对路径>", "commands": ["<修改命令1>","<修改命令2>",...]}，使用sed命令，要传入要修改的文件路径。
5. 如果多次尝试失败且仍无法解决问题，你可以通过上网搜索寻找解决方案，搜索内容要符合搜索的风格，输出格式为 {"reason": "<推理过程>", "action": "search", "content": "<搜索内容>"}，但是请注意，只有在尝试自身思考多次无果后才执行搜索。
请注意，每次尝试之后，你必须基于新的信息继续思考或修复，确保你在执行搜索之前已经尽力排查和解决问题。每次只能执行一次命令或一次搜索。
6. 如果修复成功，输出所有有效解决问题的终端命令 {"reason": "<推理过程>", "action": "finish", "content": "success", "summary": "<解决方案的描述和总结>", "commands":["<命令1>","命令2",...]}。
7. 如果多次尝试之后仍然失败，无法完成初始命令的目标，请输出 {"reason": "<推理过程>", "action": "finish", "content": "fail"}，注意一定要经过多次尝试后均失败才算失败。`},
		];
...
上一步动作输出是 ${output}`;
	...
	segs["Action"]=Action=async function(input){//:1IUIQ6VP90
		...
		if(input.action==="modify"){
			return {seg:ModifyFile,result:(input),preSeg:"1IUIQ6VP90",outlet:"1J0T883LJ0"};
		}
	...
	segs["BackPath"]=BackPath=async function(input){//:1IVIPCBLH0
		...
		commands.push(`cd "${current_path}"`);
		result=`执行 ${command} 失败，错误为 ${issue}，修复流程：${commands}`;
		/*}#1IVIPCBLH0PostCodes*/
	...
	segs["Issues"]=Issues=async function(input){//:1J08OTLLI0
		...
		result=trimJSON(result);
		return {seg:ListFiles,result:(result),preSeg:"1J08OTLLI0",outlet:"1J08P3QKK0"};
	};
	Issues.jaxId="1J08OTLLI0"
	Issues.url="Issues@"+agentURL
	
	segs["Search"]=Search=async function(input){//:1J0B5OQ2H0
		let prompt;
		let result=null;
		/*#{1J0B5OQ2H0Input*/
		/*}#1J0B5OQ2H0Input*/
		...
		let chatMem=Search.messages
		let seed="";
		if(seed!==undefined){opts.seed=seed;}
		let messages=[
			{role:"system",content:`你是一个项目部署专家，你在执行 ${command} 时遇到了问题，在google上搜索 ${bug}，请根据搜索结果总结解决方案，以JSON格式输出，如 {"solution":"You need to install ffmpeg via brew."}`},
		];
		/*#{1J0B5OQ2H0PrePrompt*/
		/*}#1J0B5OQ2H0PrePrompt*/
		prompt=`搜索结果：\n${input}`;
		if(prompt!==null){
			...
			/*#{1J0B5OQ2H0FilterMessage*/
			/*}#1J0B5OQ2H0FilterMessage*/
			messages.push(msg);
		}
		/*#{1J0B5OQ2H0PreCall*/
		/*}#1J0B5OQ2H0PreCall*/
		result=(result===null)?(await session.callSegLLM("Search@"+agentURL,opts,messages,true)):result;
		result=trimJSON(result);
		/*#{1J0B5OQ2H0PostCall*/
		search_result=result.solution;
		/*}#1J0B5OQ2H0PostCall*/
		return {result:result};
	};
	Search.jaxId="1J0B5OQ2H0"
	...
	segs["SearchIssue"]=SearchIssue=async function(input){//:1J0BMLV7G0
		let result=input
		/*#{1J0BMLV7G0Code*/
		bug = input.search_content;
		result=await fetchLinksAndArticles(input.search_content);
		/*}#1J0BMLV7G0Code*/
		return {seg:Search,result:(result),preSeg:"1J0BMLV7G0",outlet:"1J0BMMCDL0"};
	};
	...
	segs["ModifyFile"]=ModifyFile=async function(input){//:1J0TPSN240
		let result,args={};
		args['bashId']=globalContext.bash;
		args['action']="Command";
		...
		output=result;
		/*}#1J0TPSN240PostCodes*/
		return {seg:Path,result:(result),preSeg:"1J0TPSN240",outlet:"1J0TPTJBK0"};
	};
	ModifyFile.jaxId="1J0TPSN240"
	ModifyFile.url="ModifyFile@"+agentURL
...
//						"platform": "\"OpenAI\"",
//						"mode": "gpt-4.1-mini",
//						"system": "#`你是一个项目部署专家，你在执行 ${command} 时遇到了问题，在google上搜索 ${bug}，请根据搜索结果总结解决方案，以JSON格式输出，如 {\"solution\":\"You need to install ffmpeg via brew.\"}`",
//						"temperature": "0",
//						"maxToken": "2000",
//						"topP": "1",
//						"fqcP": "0",
