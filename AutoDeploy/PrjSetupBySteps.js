...
let PrjSetupBySteps=async function(session){
	...
	let self;
	let Start,InitEnv,InitPrj,LoadSteps,HasSteps,LoadGuide,LoopSteps,SwitchAction,RunBrew,RunConda,RunTaskBot,CheckStepReuslt,SaveConda,ExposeAgent,ShowError,AskRetry,AbortStep,CheckGuide,TipNoGuide,TipStep,HfDownLoad,AddNote,LoadRagGuide,CheckRagGuide,ShowGuide,TipPubFinish,GenGuideJs,Export,goto,RunCommand,Summary,OutputGuide,FixArgs,Path;
	let env=null;
	let project=null;
	let steps=null;
	let setupGuide="";
	...
	/*#{1IJ2K5IBR0LocalVals*/
	let current_command, current_check_command, current_command_output, current_path, deploy_issues="", deploy_guide;
	/*}#1IJ2K5IBR0LocalVals*/
	...
	segs["LoopSteps"]=LoopSteps=async function(input){//:1IJ2KJ0UK0
		...
		return {seg:Summary,result:(result),preSeg:"1IJ2KJ0UK0",outlet:"1IJ2L5O0R1"};
	};
	...
	segs["SaveConda"]=SaveConda=async function(input){//:1IJEP433U0
		...
		/*}#1IJEP433U0Code*/
		return {seg:ExposeAgent,result:(result),preSeg:"1IJEP433U0",outlet:"1IJEP51F20"};
	};
	...
	segs["CheckGuide"]=CheckGuide=async function(input){//:1IJ2ST21N0
		...
		if(setupGuide==="a"){
			/*#{1IJ2STPIS1Codes*/
			/*}#1IJ2STPIS1Codes*/
			return {seg:ShowGuide,result:(input),preSeg:"1IJ2ST21N0",outlet:"1IJ2STPIS1"};
		}
		/*#{1IJ2ST21N0Post*/
		/*}#1IJ2ST21N0Post*/
		return {seg:LoadRagGuide,result:(result),preSeg:"1IJ2ST21N0",outlet:"1IJ2STPIS2"};
	};
	...
	segs["AddNote"]=AddNote=async function(input){//:1IJI789IM0
		...
		/*}#1IJI789IM0Code*/
		return {seg:SaveConda,result:(result),preSeg:"1IJI789IM0",outlet:"1IJI7915T0"};
	};
	AddNote.jaxId="1IJI789IM0"
	AddNote.url="AddNote@"+agentURL
	
	segs["LoadRagGuide"]=LoadRagGuide=async function(input){//:1IJL38S9B0
		let result;
		let arg={"address":ragAddress,"query":"","tags":""};
		let agentNode=("AgentBuilder")||null;
		let sourcePath="RagQuery.js";
		let opts={secrect:false,fromAgent:$agent,askUpwardSeg:null};
		/*#{1IJL38S9B0Input*/
		/*}#1IJL38S9B0Input*/
		result= await session.callAgent(agentNode,sourcePath,arg,opts);
		...
		/*}#1IJL38S9B0Output*/
		return {seg:CheckRagGuide,result:(result),preSeg:"1IJL38S9B0",outlet:"1IJL3EQAN0"};
	};
	LoadRagGuide.jaxId="1IJL38S9B0"
	LoadRagGuide.url="LoadRagGuide@"+agentURL
	
	segs["CheckRagGuide"]=CheckRagGuide=async function(input){//:1IJL3C2S10
		let result=input;
		if(!!setupGuide){
			return {seg:ShowGuide,result:(input),preSeg:"1IJL3C2S10",outlet:"1IJL3EQAN1"};
		}
		return {seg:TipNoGuide,result:(result),preSeg:"1IJL3C2S10",outlet:"1IJL3EQAN2"};
	};
	CheckRagGuide.jaxId="1IJL3C2S10"
	CheckRagGuide.url="CheckRagGuide@"+agentURL
	...
	segs["TipPubFinish"]=TipPubFinish=async function(input){//:1IK7U888P0
		...
		let opts={txtHeader:($agent.showName||$agent.name||null),channel:channel};
		let role="assistant";
		let content=(($ln==="CN")?("项目部署成功。"):("Project deployment successful."));
		session.addChatText(role,content,opts);
		return {result:result};
	};
...
10. If test with command that is impossible to terminate automatically, such as start a web service or api service, you need to set a timeout with 2 minute.`},
	...
	segs["Summary"]=Summary=async function(input){//:1IVJ2JPN90
		...
		if(seed!==undefined){opts.seed=seed;}
		let messages=[
			{role:"system",content:"请根据部署项目流程和其中部分运行命令存在的报错和解决方法，总结一份全面的部署指南。逐条列出每一步的运行命令，并指出如果遇到某个报错，如何解决，以markdown格式呈现。"},
		];
...
//						"text": {
//							"type": "string",
//							"valText": "Project deployment successful.",
//							"localize": {
//								"EN": "Project deployment successful.",
//								"CN": "项目部署成功。"
//							},
//							"localizable": true
//						},
...
//						"platform": "Claude",
//						"mode": "claude-3-7-sonnet-latest",
//						"system": "请根据部署项目流程和其中部分运行命令存在的报错和解决方法，总结一份全面的部署指南。逐条列出每一步的运行命令，并指出如果遇到某个报错，如何解决，以markdown格式呈现。",
//						"temperature": "0",
//						"maxToken": "2000",
//						"topP": "1",
//						"fqcP": "0",
